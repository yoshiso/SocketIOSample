...
{
    // localhost:3000に接続開始
    [self.socketIO connectToHost:@"192.168.33.12"
                          onPort:3000
                      withParams:[NSDictionary dictionaryWithObjectsAndKeys:@"eb80a0384f344d9c075c9b0d04fd66684810c091", @"session_token", nil]];
}
...
- (IBAction)sendEvent:(id)sender
{
    ...
    // イベント送信
    [self.socketIO sendEvent:@"comment:execute" withData:@{@"content" : self.formCell.textField.text,
                                                           @"program_id":@"1"}];
...
{
    NSLog(@"%s", __func__);
    [self.socketIO sendEvent:@"select_theater:execute" withData:@{@"theater" : @"1"}];
...
- (void)socketIO:(SocketIO *)socket didReceiveEvent:(SocketIOPacket *)packet
{
    ...
    if ([packet.name isEqualToString:@"message"]) {
        // メッセージが空でなければ追加
        if (packet.args[0][@"content"]) {
            [self.datas insertObject:packet.args[0][@"content"] atIndex:0];
